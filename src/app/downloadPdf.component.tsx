...
const DownloadPdfFormContainer: React.StatelessComponent<any> = props => (
  <section className={theme.container} id={theme.id}>
    <div className={theme.body}>
      <div className={theme.body_container}>
        ...
        <h2 className={theme.subtitle}>
          Vamos a necesitar el XML de tu estudio energético y el código de la gestión que te dio la
          administración cuando inscribiste el código.
        </h2>
        <form className={theme.form} onSubmit={props.onSubmit}>
          {props.children}
        </form>
      </div>
    </div>
  ...
  return (
    <div className={fileInputTheme.container}>
      <label className={fileInputTheme.label}>
        <input className={fileInputTheme.input} onChange={onChange} type="file" />
        <span className={fileInputTheme.button}>
          <span className={fileInputTheme.icon_wrapper}>
            <i className={fileInputTheme.icon} />
          </span>
...
const InputReferencia: React.StatelessComponent<InputReferenciaProps> = props => {
  return (
    <div className={inputReferenciaTheme.container}>
      <label className={inputReferenciaTheme.label}>{inputReferenciaTheme.labelContentText}</label>
      <div className={inputReferenciaTheme.inputContainer}>
        <input
          className={inputReferenciaTheme.input}
          type="text"
          placeholder={inputReferenciaTheme.inputPlaceholder}
          onChange={props.onChange}
        />
      </div>
      <p className={inputReferenciaTheme.help}>{inputReferenciaTheme.helpContentText}</p>
    </div>
  );
};
...
export class DownloadPdfForm extends React.Component<{}, State> {
  ...
  render() {
    ...
    return (
      <DownloadPdfFormContainer onSubmit={this.onSubmit.bind(this)}>
        <FileInput fileName={this.state.fileName} onChange={this.onFileChange.bind(this)} />
        <ErrorText formError={this.state.formError} />
        <InputReferencia onChange={this.onNumReferenciaChange.bind(this)} />
        <InputButton disabled={this._disabledForm} onSubmit={this.onSubmit.bind(this)} />
